		...
		<form class="row py-3" enctype="multipart/form-data" method="POST" action="{{route('brands.update',$brand->id)}}">
			...
			<div class="offset-md-2 col-md-8">
				<div class="form-group">
					<input type="text" name="name" placeholder="Name" class="form-control {{ $errors->has('name') ? 'border border-danger' : '' }}" value="{{$brand->name}}">
					<span style="color:red;">{{$errors->first('name')}}</span>
				</div>
				<div class="form-group">
					<input type="file" name="logo" placeholder="Logo" class="form-control-file pb-2 {{ $errors->has('logo') ? 'border border-danger' : '' }}" value="{{$brand->logo}}">
